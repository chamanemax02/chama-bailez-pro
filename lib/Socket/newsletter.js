...
export const makeNewsletterSocket = (config) => {
    ...
    return {
        ...
        newsletterSendPollVote: async (jid, parentServerId, options) => {
            ...
            return { id: messageId, ack };
        },
        newsletterFetchMessages: async (typeOrJid, countOrKey = 20, after, before) => {
            let type = 'jid';
            let key = typeOrJid;
            let count = countOrKey;
            if (typeOrJid === 'invite' || typeOrJid === 'jid') {
                type = typeOrJid;
                key = countOrKey;
                count = after || 20;
            }
        ...
        newsletterMyAddOns: async (jid) => {
            const result = await query({
                tag: 'iq',
                attrs: { id: generateMessageTag(), type: 'get', xmlns: 'newsletter', to: S_WHATSAPP_NET },
                content: [{ tag: 'my_addons', attrs: { limit: '100', ...(jid ? { jid } : {}) }, content: undefined }]
            });
            return result;
        },
