...
export const makeMessagesSocket = (config) => {
    ...
    const userDevicesCache = config.userDevicesCache ||
        new NodeCache({
            stdTTL: DEFAULT_CACHE_TTLS.USER_DEVICES, // 3 minutes
            useClones: false,
            maxKeys: 250
        });
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isNewsletter) {
                ...
                const normalizedM = normalizeMessageContent(message);
                const innerMsg = normalizedM?.[mediaType + 'Message'] || normalizedM?.[mediaType];
                const mediaId = innerMsg?.mediaHandle || additionalAttributes?.media_id;
                const stanza = {
                    tag: 'message',
                    attrs: {
                        to: jid,
                        ...
                        ...(mediaId ? { media_id: String(mediaId) } : {}),
                        ...(additionalAttributes || {})
                    },
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                const isInteractiveMsg = ('interactiveMessage' in content && !!content.interactiveMessage) ||
                                         ('interactive' in content && !!content.interactive) ||
                                         ('buttons' in content && !!content.buttons);
        ...
        sendButtons: async (jid, buttonContent, options = {}) => {
            return sock.sendMessage(jid, buttonContent, options);
        },
        /**
         * Ultra-fast zero-download forward message to ANY chat (group, DM, or channel/newsletter).
         * Supports media (videos, images, docs, audio, stickers), view-once, and text with custom captions.
         * @param {string} jid Target JID (including @newsletter, @g.us, @s.whatsapp.net, etc.)
         * @param {any} message Message to forward (can be a full WAMessage or raw message object)
         * @param {object} options Forwarding options (e.g. caption, forceForward, quoting, etc.)
         */
        forwardMessage: async (jid, message, options = {}) => {
            const meId = assertMeId(authState.creds);
            let content = message?.message || message;
            if (!content) {
                throw new Boom('no content in message to forward', { statusCode: 400 });
            }

            // Recursively unwrap wrappers like ephemeral, viewOnce, documentWithCaption
            ...
            // Clone message cleanly
            unwrapped = proto.Message.decode(proto.Message.encode(unwrapped).finish());

            const mediaTypes = ['documentMessage', 'imageMessage', 'videoMessage', 'audioMessage', 'stickerMessage'];
            const mediaType = mediaTypes.find((t) => unwrapped[t]);
            const isMedia = Boolean(mediaType);

            // Handle caption override
            if (options.caption !== undefined && options.caption !== null) {
                if (isMedia && unwrapped[mediaType]) {
                    unwrapped[mediaType].caption = options.caption;
                } else if (unwrapped.extendedTextMessage) {
                    unwrapped.extendedTextMessage.text = options.caption;
                } else if (unwrapped.conversation) {
                    unwrapped.extendedTextMessage = { text: options.caption };
                    delete unwrapped.conversation;
                }
            }

            // ContextInfo / Forwarding metadata
            const targetMsg = isMedia ? unwrapped[mediaType] : (unwrapped.extendedTextMessage || unwrapped.conversation ? (unwrapped.extendedTextMessage || unwrapped) : null);
            if (targetMsg && typeof targetMsg === 'object') {
                targetMsg.contextInfo = targetMsg.contextInfo || {};
                if (options.forceForward !== false) {
                    targetMsg.contextInfo.isForwarded = true;
                    targetMsg.contextInfo.forwardingScore = (targetMsg.contextInfo.forwardingScore || 0) + 1;
                }
            }
            ...
            if (isNewsletter) {
                ...
                await relayMessage(jid, unwrapped, { messageId: msgId, ...options });
                return { key: { remoteJid: jid, fromMe: true, id: msgId }, message: unwrapped };
            }

            const waMsg = generateWAMessageFromContent(jid, unwrapped, {
                userJid: meId,
