...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    else if ('jimp' in lib && (lib.jimp?.Jimp || lib.jimp?.default || lib.jimp)) {
        const JimpClass = lib.jimp?.Jimp || lib.jimp?.default || lib.jimp;
        const jimp = typeof JimpClass.read === 'function' ? await JimpClass.read(bufferOrFilePath) : await JimpClass(bufferOrFilePath);
        const dimensions = {
            width: jimp.width || jimp.bitmap?.width || 100,
            height: jimp.height || jimp.bitmap?.height || 100
        };
        let resized = jimp;
        try {
            if (typeof jimp.resize === 'function') {
                resized = jimp.resize({ w: width, mode: lib.jimp.ResizeStrategy?.BILINEAR });
            }
        } catch {
            try { resized = jimp.resize(width, -1); } catch {}
        }
...
export const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs, newsletter }) => {
        ...
        for (const { hostname } of hosts) {
            ...
            const mediaPath = newsletter ? `/newsletter/newsletter-${mediaType}` : MEDIA_PATH_MAP[mediaType];
            ...
            try {
                ...
                if (result?.url || result?.direct_path) {
                    urls = {
                        ...
                        fbid: result.fbid,
                        handle: result.handle || result.media_id || result.fbid,
                        ts: result.ts
                    };
                    break;
                }
