...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        ...
        const safeParseParticipant = (a) => {
            if (!a) return {};
            if (typeof a === 'object') return a;
            try {
                const parsed = JSON.parse(a);
                if (typeof parsed === 'object' && parsed !== null) return parsed;
                return { phoneNumber: parsed, pn: parsed, lid: parsed };
            } catch {
                return { phoneNumber: a, pn: a, lid: a };
            }
        };
        const participantsIncludesMe = () => participants.find(jid => areJidsSameUser(meId, jid?.phoneNumber || jid?.pn || jid?.lid || jid)); // ADD SUPPORT FOR LID
        switch (message.messageStubType) {
            case WAMessageStubType.GROUP_PARTICIPANT_CHANGE_NUMBER:
                participants = (message.messageStubParameters || []).map(safeParseParticipant);
            ...
            case WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD: // TODO: Add other events
                const participant = safeParseParticipant(message.messageStubParameters?.[0]);
