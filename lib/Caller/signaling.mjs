...
export class SignalingBridge {
    ...
    discoverPeerDevices = async (peerLidJid, targetPnJid) => {
        let devices = [];
        const toQuery = [targetPnJid, peerLidJid].filter(Boolean);
        try {
            devices = await this.#sock.getUSyncDevices(toQuery, true, false);
        } catch {}
        ...
        const fallback = [];
        if (targetPnJid) fallback.push(`${this.#toBareJid(targetPnJid).split('@')[0]}:0@s.whatsapp.net`);
        if (peerLidJid) fallback.push(`${this.#toBareJid(peerLidJid).split('@')[0]}:0@lid`);
        return fallback;
    };
